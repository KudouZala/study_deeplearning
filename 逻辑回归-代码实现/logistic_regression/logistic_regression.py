...
class LogisticRegression:
    def __init__(self,data,labels,polynomial_degree = 0,sinusoid_degree = 0,normalize_data=False):
        ...
        self.labels = labels#labels是y_train
        self.unique_labels = np.unique(labels)#计算有多少种labels，即要分多少类
        ...
        num_unique_labels = np.unique(labels).shape[0]#行数
        self.theta = np.zeros((num_unique_labels,num_features))#theta是一个a行b列的矩阵，a代表着要分的种类数，b代表特征数
        
    def train(self,max_iterations=1000):
        cost_histories = []
        num_features = self.data.shape[1]
        for label_index,unique_label in enumerate(self.unique_labels):#输出：0，红色；1，蓝色；2，绿色...
            current_initial_theta = np.copy(self.theta[label_index].reshape(num_features,1))
            current_lables = (self.labels == unique_label).astype(float) # 将标签转换为浮点数形式,第一次就区分是红色给1、不是红色给0；因此相当于是做二分类
